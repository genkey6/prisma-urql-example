...
export class FamilyDatasource implements FamilyRepository {
  ...
  async findById(id: string): Promise<Family | undefined> {
    ...
    const classis = Classis.reconstruct(family.order.classis.id, family.order.classis.name);
    const order = Order.reconstruct(family.order.id, family.order.name, classis);
