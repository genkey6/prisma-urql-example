...
export class AnimalDatasource implements AnimalRepository {
  constructor(
    @Inject(DATASOURCE_CLIENT) private client: DatasourceClient,
  ) {}
  ...
  async findMany(): Promise<Animal[]> {
    ...
    return animals.map((animal) => {
      const classis = Classis.reconstruct(animal.family.order.classis.id, animal.family.order.classis.name);
      const order = Order.reconstruct(animal.family.order.id, animal.family.order.name, classis);
      const family = Family.reconstruct(animal.family.id, animal.family.name, order);
